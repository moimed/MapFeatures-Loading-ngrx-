...
export const mapFeaturesReducer = createReducer(
    ...
    on(addSelectedMapFeature, (state, {selectedFeature}) => ({...state, selectedFeatures:
    ...
    on(removeAllSelectedMapFeatures, (state, {featureType}) => ({...state, selectedFeatures: state.selectedFeatures.filter(feature => feature.type !== featureType)
    })),    
);